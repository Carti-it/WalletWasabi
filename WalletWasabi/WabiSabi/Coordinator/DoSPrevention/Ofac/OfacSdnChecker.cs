...
public class OfacSdnChecker : PeriodicRunner
{
	...
	protected override async Task ActionAsync(CancellationToken cancellationToken)
	{
		try
		{
			...
			var list = await _ofacParser.GetSanctionedBtcAddressesAsync(xmlStream, cancellationToken).ConfigureAwait(false);

			// Regtest test.
			// List<string> list = ["bcrt1qytypjcqy7gdv5lmz95q04r0tus3ehpty7nmptd"];
		...
		{
			Logger.LogError($"Failed to retrieve OFAC sanctions list: {e}");
		}
	}
